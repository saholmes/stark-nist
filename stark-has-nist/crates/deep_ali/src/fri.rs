...
//
//  Uses `TowerField::inverse` so the trait does not need `Div` as a
//  supertrait.  Concrete types that implement `Div` can still use `/`
//  directly — this helper is for the generic code paths.
// ────────────────────────────────────────────────────────────────────────
...
//
//  Drawing folding challenges from E gives exception probability
//  L_ℓ / |E| per layer.  For E = F_{p^d} over Goldilocks this is
//  L_ℓ · 2^{-64d}, e.g. ≈ 2^{-128} for d=2 or ≈ 2^{-192} for d=3.
// ────────────────────────────────────────────────────────────────────────

/// DEEP quotient for an **extension-field** codeword.
///
/// Given f_ℓ : H_ℓ → E as evaluations at base-field domain points,
/// computes Q_ℓ(x) = (f_ℓ(x) − f_ℓ(z)) / (x − z) for all x ∈ H_ℓ.
///
/// The polynomial interpolation is performed componentwise: E::DEGREE
/// independent base-field iFFTs recover the E-valued coefficients,
/// then Horner evaluation at z ∈ E uses true extension-field
/// multiplication.
fn compute_q_layer_ext_on_ext<E: TowerField>(
    ...
    // ── Componentwise iFFT ──
    // Extract d vectors of base-field evaluations, one per Fp-coordinate.
...
/// Fold an extension-field codeword with challenge α ∈ E.
///
///   f_{ℓ+1}[b] = Σ_{j=0}^{m-1}  f_ℓ[b + j·n_next] · α^j
///
/// All arithmetic is in E.
...
fn challenge_ext<E: TowerField>(tr: &mut Transcript, tag: &[u8]) -> E {
    ...
    for i in 0..d {
        ...
        // Append the component index as ASCII decimal
...
pub fn fri_build_transcript<E: TowerField>(
    ...
    // α₀ ∈ E, all subsequent layers are in E.
...
pub fn fri_prove_queries<E: TowerField>(
    ...
    r: usize,
    query_seed: F,
) -> (Vec<FriQueryOpenings<E>>, Vec<[u8; 32]>, FriLayerProofs, Vec<MerkleOpening>) {
...
mod tests {
    ...
    // ── Import the concrete cubic extension type for tests ──
    // It must implement TowerField (via the config / Option A approach).
    ...
    // ── Extension-field fold correctness test (generic helper) ──
    ...
    fn test_ext_fold_preserves_low_degree_with<E: TowerField>() {
        ...
        // Build a random challenge in E from DEGREE base-field elements
